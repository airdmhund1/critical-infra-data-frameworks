 ...
 *
 * Callers receive an `Either[ConfigLoadError, SourceConfig]` from the loader.
 * Pattern matching on the left projection gives exhaustive, compile-checked
 * error handling without any exception leakage.
 */
 ...
 *
 * This covers required-field absence, type mismatches, enum constraint
 * violations, and conditional-field rule failures (e.g., `incrementalColumn`
 * missing when `mode` is `incremental`).
 *
 * @param field   Dot-separated path to the offending field (e.g., `"ingestion.incrementalColumn"`).
 ...
 *
 * A resolution failure means the external secrets manager (HashiCorp Vault or
 * AWS KMS) returned an error or the reference path does not exist. The pipeline
 * must not start if credentials cannot be resolved.
 *
 * @param ref   The `credentialsRef` value that could not be resolved
 *              (e.g., `"vault://secret/data/oracle-tradedb-prod"`).
 * @param cause Human-readable description of why resolution failed.
 */
 ...
 *
 * @param message Human-readable description of the parse failure, including position
 *                information where the underlying parser provides it.
 */
