...
/** In-memory test double for [[SecretsResolver]] backed by a pre-configured map.
  *
  * Construct an instance with every `ref → value` pair that the test scenario requires. Any
  * `resolve` call whose `ref` is not present in the map returns a [[SecretsResolutionError]],
  * allowing tests to exercise both happy-path and error-path branches without a live secrets
  * manager.
  *
  ...
  *
  * @param secrets
  *   Pre-configured map of secrets reference strings to plaintext values.
  */
final class MockSecretsResolver(secrets: Map[String, String]) extends SecretsResolver {

  /** Returns the plaintext value for `ref` if it exists in the pre-configured map, or a
    * [[SecretsResolutionError]] if it does not.
    *
    * @param ref
    *   The secrets reference to look up.
    * @return
    *   `Right(value)` if `ref` is present in the map, or `Left(SecretsResolutionError(ref,
